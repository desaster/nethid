...
export const TKL_MAIN: KeyboardLayout = {
    ...
    name: 'Main',
    kle: [
        ["Esc",{x:1},"F1","F2","F3","F4",{x:0.5},"F5","F6","F7","F8",{x:0.5},"F9","F10","F11","F12",{x:0.25},"PrtSc","ScrlL","Pause\nBreak"],
        [{y:0.5},"~\n`","!\n1","@\n2","#\n3","$\n4","%\n5","^\n6","&\n7","*\n8","(\n9",")\n0","_\n-","+\n=",{w:2},"Backspace",{x:0.25},"Insert","Home","PgUp",{x:0.25},"NumL","/","*","-"],
    ...
    mapping: [
        ...
        hid(HID.NUM_LOCK), hid(HID.KP_DIVIDE), hid(HID.KP_MULTIPLY), hid(HID.KP_MINUS),
        // Row 2
        ...
        hid(HID.KP_7), hid(HID.KP_8), hid(HID.KP_9), hid(HID.KP_PLUS),
        // Row 3
        ...
        hid(HID.KP_4), hid(HID.KP_5), hid(HID.KP_6),
        // Row 4
        ...
        hid(HID.KP_1), hid(HID.KP_2), hid(HID.KP_3), hid(HID.KP_ENTER),
        // Row 5
...
export const MEDIA_LAYOUT: KeyboardLayout = {
    ...
    name: 'Extra',
    kle: [
        [{w:1.25},"Prev",{w:1.25},"Play/Pause",{w:1.25},"Next",{w:1.25},"Stop",{x:0.75,w:1.25},"Power"],
        [{y:0.25,w:1.25},"Mute",{x:0.75,w:1.5},"Vol -",{w:1.5},"Vol +",{x:0.75,w:1.25},"Sleep"],
        [{y:0.25,w:1.25},"Back",{w:1.25},"Forward",{w:1.25},"Refresh",{w:1.25},"Home",{x:0.75,w:1.25},"Wake"]
    ],
    mapping: [
        consumer(CONSUMER.PREV_TRACK), consumer(CONSUMER.PLAY_PAUSE), consumer(CONSUMER.NEXT_TRACK), consumer(CONSUMER.STOP), system(SYSTEM.POWER),
        consumer(CONSUMER.MUTE), consumer(CONSUMER.VOLUME_DOWN), consumer(CONSUMER.VOLUME_UP), system(SYSTEM.SLEEP),
        consumer(CONSUMER.WWW_BACK), consumer(CONSUMER.WWW_FORWARD), consumer(CONSUMER.WWW_REFRESH), consumer(CONSUMER.WWW_HOME), system(SYSTEM.WAKE),
    ],
};
...
export const MOBILE_ABC: KeyboardLayout = {
    ...
    kle: [
        ...
        [{ w: 1.5 }, "Shift", "Z", "X", "C", "V", "B", "N", "M", { w: 1.5 }, "⌫"],
...
export const MOBILE_123: KeyboardLayout = {
    ...
    kle: [
        ["1","2","3","4","5","6","7","8","9","0"],
        ["-","=","[","]","\\",";","'",",",".","/"],
        [{w:1.5},"Shift",{x:0.25,w:1.25},"`",{w:1.25},"Tab",{w:1.25},"Esc",{w:1.25},"Ins",{w:1.25},"Del",{x:0.5,w:1.5},"⌫"],
        [{w:1.5},"Ctrl",{w:1.5},"Alt",{a:7,w:5.5},"",{a:4,w:1.5},"Enter"]
    ],
    mapping: [
        ...
        mod(HID.LSHIFT), hid(HID.GRAVE), hid(HID.TAB), hid(HID.ESCAPE), hid(HID.INSERT), hid(HID.DELETE), hid(HID.BACKSPACE),
        mod(HID.LCTRL), mod(HID.LALT), hid(HID.SPACE), hid(HID.ENTER),
    ],
};
...
export const MOBILE_FN: KeyboardLayout = {
    ...
    name: 'Fn',
    kle: [
        ["F1","F2","F3","F4",{x:0.5},"F5","F6","F7","F8"],
        ["F9","F10","F11","F12",{x:2.5},"Up"],
        [{w:1.5},"Shift",{x:4},"Left","Down","Right"],
        [{w:1.5},"Ctrl",{w:1.5},"Alt",{a:7,w:4},"",{a:4,w:1.5},"Enter"]
    ],
    mapping: [
        hid(HID.F1), hid(HID.F2), hid(HID.F3), hid(HID.F4),
        hid(HID.F5), hid(HID.F6), hid(HID.F7), hid(HID.F8),
        hid(HID.F9), hid(HID.F10), hid(HID.F11), hid(HID.F12), hid(HID.UP),
        mod(HID.LSHIFT), hid(HID.LEFT), hid(HID.DOWN), hid(HID.RIGHT),
        mod(HID.LCTRL), mod(HID.LALT), hid(HID.SPACE), hid(HID.ENTER),
    ],
};
...
export const MOBILE_MEDIA: KeyboardLayout = {
    ...
    name: 'Extra',
    kle: [
        [{w:1.5},"Prev",{w:1.5},"Play",{w:1.5},"Next",{w:1.5},"Stop",{x:1,w:1.25},"Power"],
        [{y:0.25,w:1.5},"Mute",{x:1.5,w:1.5},"Vol -",{w:1.5},"Vol +",{x:1,w:1.25},"Sleep"],
        [{y:0.25,w:1.5},"Back",{w:1.5},"Forward",{w:1.5},"Refresh",{w:1.5},"Home",{x:1,w:1.25},"Wake"]
    ],
    mapping: [
        consumer(CONSUMER.PREV_TRACK), consumer(CONSUMER.PLAY_PAUSE), consumer(CONSUMER.NEXT_TRACK), consumer(CONSUMER.STOP), system(SYSTEM.POWER),
        consumer(CONSUMER.MUTE), consumer(CONSUMER.VOLUME_DOWN), consumer(CONSUMER.VOLUME_UP), system(SYSTEM.SLEEP),
